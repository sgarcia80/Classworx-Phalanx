...
{
    public class vwPwdRqstGrpFactory
    {
        ...
        public bool? FilGrupoActivo = null;

        public vwPwdRqstGrpEntityCollection GetAll()
        {
            IList<vwPwdRqstGrpEntity> lstWLUs;
            vwPwdRqstGrpEntityCollection DBUsrEC = new vwPwdRqstGrpEntityCollection();
            using (ISession session = DBMgr.factory.OpenSession())
            {
                ICriteria DataSearch = session.CreateCriteria(typeof(vwPwdRqstGrpEntity));
                if (FilUsername != "")
                {
                ...
                if (FilGrupoActivo != null)
                {
                    DataSearch.Add(Expression.Eq("GrupoActivo", FilGrupoActivo.Value));
                }
                if (OrderByFolio)
                {
                ...
                lstWLUs = DataSearch.List<vwPwdRqstGrpEntity>();
                DBUsrEC.Add(lstWLUs);
