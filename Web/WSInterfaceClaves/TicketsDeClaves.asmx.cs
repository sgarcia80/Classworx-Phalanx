...
namespace WSInterfaceClaves
{
    /// <summary>
    /// Descripción breve de TicketsDeClaves
    /// </summary>
    ...
    public class TicketsDeClaves : System.Web.Services.WebService
    {
        ...
        public AgregarTicketResultado AgregarTicket(TicketNotificacionClave ticket)
        {
            ...
            TraceHelper.Information("NumeroSolicitud: '{0}'", ticket.IdSolicitud.HasValue ? ticket.IdSolicitud.GetValueOrDefault().ToString() : string.Empty);
            TraceHelper.Information("Aplicacion: '{0}'", string.IsNullOrEmpty(ticket.CodigoAplicacion) ? string.Empty : ticket.CodigoAplicacion);
            TraceHelper.Information("Usuario Aplicacion: '{0}'", string.IsNullOrEmpty(ticket.UsuarioAplicacion) ? string.Empty : ticket.UsuarioAplicacion);
            TraceHelper.Information("Dominio: '{0}'", string.IsNullOrEmpty(ticket.DominioUsuario) ? string.Empty : ticket.DominioUsuario);
            TraceHelper.Information("Usuario: '{0}'", string.IsNullOrEmpty(ticket.Usuario) ? string.Empty : ticket.Usuario);
            TraceHelper.Information("Legajo: '{0}'", string.IsNullOrEmpty(ticket.Legajo) ? string.Empty : ticket.Legajo);
            TraceHelper.Information("TipoDocumento: '{0}'", string.IsNullOrEmpty(ticket.TipoDocumento) ? string.Empty : ticket.TipoDocumento);
            TraceHelper.Information("Documento: '{0}'", string.IsNullOrEmpty(ticket.Documento) ? string.Empty : ticket.Documento);
            TraceHelper.Information("EsPasswordDominio: '{0}'", ticket.UsaPasswordDominio ? "Si" : "No");
            TraceHelper.Information("NumeroSolicitudAltaApp: '{0}'", ticket.SolicitudID.HasValue ? ticket.SolicitudID.Value.ToString() : string.Empty);
            TraceHelper.Information("CodigoGerenciaDestino: '{0}'", string.IsNullOrEmpty(ticket.CodigoGerencia) ? string.Empty : ticket.CodigoGerencia);
            TraceHelper.Information("NombreGerenciaDestino: '{0}'", string.IsNullOrEmpty(ticket.NombreGerencia) ? string.Empty : ticket.NombreGerencia);
            TraceHelper.Information("CodigoAreaDestino: '{0}'", string.IsNullOrEmpty(ticket.SiglaArea) ? string.Empty : ticket.SiglaArea);
            TraceHelper.Information("NombreAreaDestino: '{0}'", string.IsNullOrEmpty(ticket.DescripcionArea) ? string.Empty : ticket.DescripcionArea);
            TraceHelper.Information("FechaVigencia: '{0}'", ticket.FechaVigDesde.HasValue ? ticket.FechaVigDesde.Value.ToString("dd/MM/yyyy") : string.Empty);
            TraceHelper.Information("NumeroLegajoEmpleadoSolicitud: '{0}'", string.IsNullOrEmpty(ticket.NroLegajoSoli) ? string.Empty : ticket.NroLegajoSoli);
            TraceHelper.Information("CodigoEmpresaSubsidiaria: '{0}'", string.IsNullOrEmpty(ticket.CodSubsidiaria) ? string.Empty : ticket.CodSubsidiaria);
            ...
            TraceHelper.Information("Se procesa el ticket {0}", ticket.IdSolicitud);
            TraceHelper.Information("Se busca la aplicación {0}", ticket.CodigoAplicacion);
            ...
            if (aplicacion == null)
            {
                ...
                TraceHelper.Information("No existe la aplicacion. Se da de alta [{0}]-[{1}]", ticket.CodigoAplicacion, ticket.NombreAplicacion);
            ...
            {
                TraceHelper.Information("Se encontro la aplicacion [{0}]", ticket.CodigoAplicacion);
                if (_debugMode)
                {
            ...
            //Si no informa Numero de Solicitud de BPM, el ticket viene de SailPoint
            if (!solicitudBPM.NumeroSolicitud.HasValue)
            {
                solicitudBPM.FechaAceptacionTyC = DateTime.Now;
            }
            ...
            bool esEmpleado = !string.IsNullOrEmpty(ticket.Legajo);

            bool HayQueInsertar = true;
            bool vieneXBPM = false;

            vieneXBPM = solicitudBPM.NumeroSolicitud.HasValue;
            ...
            if (vieneXBPM)
            {
                ...
                TicketNotificacionClaveEntity ticketOriginal = bsolb.GetDuplicado(ticket.IdSolicitud.GetValueOrDefault(), aplicacion);

                if (ticketOriginal != null)
                {
                    TraceHelper.Information("Ya existe el ticket {0}", ticket.IdSolicitud);
                    ...
                    else
                    {
                        ...
                        {
                            TraceHelper.Information("Si el ticket {0} no fue visto, se actualiza", ticket.IdSolicitud);
                        ...
                        {
                            TraceHelper.Information("Se actualiza la marca de corregido del ticket {0}", ticket.IdSolicitud);
            ...
            if (vieneXBPM)
            {
                ...
                {
                    DateTime fechadesde = DateTime.Today.AddMonths(-6);

                    TraceHelper.Information("Es Alta de Red o App Notificable");

                    TraceHelper.Information("Se consulta si el usuario '{0}' ya se notifico para la aplicacion '{1}'", ticket.UsuarioAplicacion, ticket.CodigoAplicacion);

                    //Se buscan si la aplicacion/usuario ya tienen algun ticket previo donde se haya notificado
                    TicketNotificacionClaveEntityCollection notificados = bsolb.GetTickesByUser(solicitudBPM.Aplicacion, solicitudBPM.DominioUsuario, solicitudBPM.Usuario, solicitudBPM.UsuarioAplicacion, 
                                                                                                solicitudBPM.TipoDocumento, solicitudBPM.Documento, !esEmpleado, fechadesde, true);

                    if (notificados != null && notificados.Count > 0)
                    {
                        TraceHelper.Information("El usuario '{0}' tiene un Alta para la app ya notificada, se cancela la nueva con 01/01/1900", ticket.UsuarioAplicacion);
                        solicitudBPM.FechaAceptacionTyC = new DateTime(1900, 1, 1);
                    }
                    else
                    {
                        TraceHelper.Information("Se consulta si el usuario '{0}' tiene una notificacion pendiente para la app '{1}'", ticket.UsuarioAplicacion, ticket.CodigoAplicacion);

                        TicketNotificacionClaveEntityCollection sinnotificar = bsolb.GetTickesByUser(solicitudBPM.Aplicacion, solicitudBPM.DominioUsuario, solicitudBPM.Usuario, solicitudBPM.UsuarioAplicacion,
                                                                                                solicitudBPM.TipoDocumento, solicitudBPM.Documento, !esEmpleado, fechadesde, false);

                        if (sinnotificar != null && sinnotificar.Count > 0)
                        {
                            TraceHelper.Information("El usuario '{0}' tiene un Alta pendiente para la app, se cancela la nueva con 01/01/1900", ticket.UsuarioAplicacion);
                            solicitudBPM.FechaAceptacionTyC = new DateTime(1900, 1, 1);
                        }
                    }
                }
            ...
            try
            {
                if (HayQueInsertar)
                {
                    solicitudBPM.ImpactaEnAD = altaUsuarioRed && vieneXBPM;

                    // verifica si es un alta de red para usuario externo
                    bool altaUsuarioRedExterno = !esEmpleado && altaUsuarioRed;

                    //Genero el token si el ticket vino por BPM y no existe un ticket previo de SailPoint
                    if (vieneXBPM && !solicitudBPM.FechaAceptacionTyC.HasValue)
                    {
                        TraceHelper.Information("El ticket {0} es de RED y {1} es externo", (altaUsuarioRed ? "SI" : "NO"), (altaUsuarioRedExterno ? "SI" : "NO"));
                        ...
                        if (altaUsuarioRedExterno)
                        {
                            ...
                            TraceHelper.Information("Se genera el token", ticket.IdSolicitud);
                    ...
                    TraceHelper.Information("Se graba el ticket {0}", ticket.IdSolicitud);
                    ...
                    // Si la notificacion vino de BPM y no tenga fecha de notificado
                    // O si la notificacion vino de SailPoint
                    if ((vieneXBPM && !solicitudBPM.FechaAceptacionTyC.HasValue) ||
                        (!vieneXBPM))
                    {
                        ...
                        if (ticket.Legajo != null && ticket.Legajo != "")
                        {
                                ...
                                || altaUsuarioRed)
                            {
                                if (ticket.CodigoAplicacion.Trim().ToLower() == appCobis.Codigo.ToLower())
                                {
                                    TraceHelper.Information("Es alta de Cobis ({0})", appCobis.Codigo);
                                }
                                if (altaUsuarioRed)
                                {
                                    TraceHelper.Information("Es alta de Red");
                                }
                                ...
                                TraceHelper.Information("Tiene que grabar ticket en Meta4");
                                ...
                                Meta4Usuarios.DireccionMail = PhalanxNAL.ActiveDirectoryHelper.BuscarEmailPorLegajoUsername(solicitudBPM.Usuario);
                                ...
                                if (altaUsuarioRed && vieneXBPM)
                                {
                                    if (_debugMode)
                                        strDebug += " | Enviando email de alta de usuario de red";

                                    TraceHelper.Information("Se envia el mail de alta de usuario de red");

                                    MailAlertBusiness MailToSendBL = new MailAlertBusiness();

                                    MailToSendBL.AltaUsuarioRedMail(Meta4Usuarios.IdUsuarioRed, solicitudBPM.NumeroSolicitud.GetValueOrDefault(), Meta4Usuarios.FechaNovedad);

                                    if (_debugMode)
                                        strDebug += " | Email enviado";
                                }
                            }
                            if (vieneXBPM && (altaUsuarioRed || aplicacion.Notificable))
                            {
                                ...
                                TraceHelper.Information("Se busca en META4 el mail del usuario (Documento {0} - {1})", ticket.TipoDocumento, ticket.Documento);
                                ...
                                {
                                    TraceHelper.Information("Se encontro en META4 y se envia mail a {0}", legajo.EMail);
                        ...
                        else
                        {
                            ...
                            if (vieneXBPM && altaUsuarioRedExterno)
                            {
                                TraceHelper.Information("Se envia mail para recurso externo");
            ...
            {
                TraceHelper.Error(ex, "Error al procesar el ticket");

                try
                {
                    if (!PasaInsertM4)
                    {
                        TraceHelper.Information("Se borra el ticket");
                        if (_debugMode)
                        {
            ...
            TraceHelper.Information(resultado.Mensaje);
        ...
        private bool EnviarEmailAltaUsuarioRedExterno(TicketNotificacionClaveEntity solicitudBPM, out string debug)
        {
            ...
            {
                TraceHelper.Information("Se busca la subsidiaria {0}", solicitudBPM.CodigoEmpresaSubsidiaria);
                ...
                if (subsidiaria == null)
                {
                    TraceHelper.Information("No se encontro la subsidiaria");

                    debug += " | No se encuentra la empresa subsidiaria con código = " + solicitudBPM.CodigoEmpresaSubsidiaria;
                }
                else
                {
                    TraceHelper.Information("Se encontro la subsidiaria [{0}]", subsidiaria.Nombre);
            ...
            if (mailTo.Count == 0)
            {
                TraceHelper.Information("No se encontro mail de la subsidiaria, se busca por Legajo {0}", solicitudBPM.NumeroLegajoEmpleadoSolicitud);
                ...
                if (email == null)
                {
                    TraceHelper.Information("No se encontro el usuario en AD");
            ...
            TraceHelper.Information("Se envia mail de alta de usuario externo");

            solicitudBPM.MailId = MailToSendBL.AltaUsuarioRedExternoMail(mailTo.ToArray(), solicitante, solicitudBPM.NumeroSolicitud.GetValueOrDefault(), solicitudBPM.Fecha, solicitudBPM.Token, destino);
